import { extractMeetupPublicFloorClaimKeys } from "./meetup-publication-policy.js";
 ...
 * Meetup description. Conflicting floor claims reject the public projection;
 * other ambiguous source wording fails closed to null. The sync never infers
 * price, age policy, or current availability.
 *
 * @param {unknown} input
...
export function extractMeetupPublicEventFacts(input, options = {}) {
  ...
  const publicFloor = hasPublicVenue
    ? extractUnambiguousPublicFloor(description)
    : null;
...
function extractUnambiguousPublicFloor(description) {
  const claims = extractMeetupPublicFloorClaimKeys(description);
  if (claims.length > 1) {
    throw new Error("Conflicting Meetup public floor claims.");
  }
  return uniqueTextMatches(
    description,
